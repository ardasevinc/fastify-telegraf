...
const fastifyTelegraf: FastifyPluginAsyncTypebox<FastifyTelegrafOptions> =
  async function (app, options) {
    ...
    if (app[decoratorBotName]) {
      throw new Error(
        `fastify-telegram: fastify.${decoratorBotName} already exists`,
      );
    }
      ...
      app.addHook('onReady', async function startPolling() {
        ...
        if (startError) {
          const fail = new Error(
            `fastify-telegram: bot.launch() failed: ${startError.message}`,
          );
    ...
    app.addHook('onReady', async function startWebHook() {
      try {
        ...
        const fail = new Error(
          `fastify-telegram: bot.createWebhook() failed: ${error.message}`,
        );
        fail.cause = error;
        throw fail;
      }
